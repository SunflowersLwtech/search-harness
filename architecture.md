...
### 3.2 零配置默认,渐进升级
- **零 key 即可大部分跑通**(Tier 1 多数 + Tier 3 + Tier 4)
- **GitHub Personal Access Token 实质必需**(匿名 code search 直接 401,实测见 [`adapter-empirical-test-report.md`](./adapter-empirical-test-report.md));非"可选升级",而是 30 秒强推 prerequisite
- **Semantic Scholar key 强推**(匿名烈性限流,burst 即触发 429)— 同 PAT 一档 prerequisite
...
**实际筛选结果在 [`adapter-vs-skill.md`](./adapter-vs-skill.md)**:Tier 1 收敛到 **22 个核心 adapter**(2026-04-29 实测后,Papers with Code 因域名死被 drop;详见 [`adapter-empirical-test-report.md`](./adapter-empirical-test-report.md)),数十个 site 转为 skill-only。
...
|---|---|---|---|---|---|---|
| **1** | 原生垂直 API(零 key 优先) | 默认 on | <1s | 极低 | ~1.0 | **22 个核心 adapter**(见 [adapter-vs-skill.md §2](./adapter-vs-skill.md)) |
...
| **4** | 准多模态(平台已文本化) | 默认 on | 1-2s | 低 | ~0.6 | **invidious `/api/v1` 多实例池**(zero-key YouTube wrapper)+ self-host fallback + yt-dlp 兜底;详见 [domain-knowledge.md §5.1 #17](./domain-knowledge.md) |
...
Recommended (2 个 key,1 分钟,**实质 prerequisite**)
  ├ GitHub Personal Access Token(code search 匿名 401,搜索仓库 60→5000 req/hr)
  └ Semantic Scholar API key(匿名 burst 即 429)
  
  → 覆盖 90%(**没有这两个 key 是 70% 而非 90%**)

Power User (多 key,按需加)
  ├ Tavily / Exa / Brave 任选 1(Tier 3 web search)
  ├ Gemini API(Tier 5 视频理解)
  ├ Reddit OAuth(Reddit Tier 1 提速 + 抗趋紧)
  ├ libraries.io API key(/search 跨包管理器搜索;detail 查询匿名可用)
  └ NVD API key(50 req/30s vs 匿名 5/30s)
...
- [x] ~~可选 adapter(libraries.io / OpenReview / HF Hub / CVE 等)是否进 Phase 1~~ → **[2026-04-26] 全部纳入 Phase 1**;**[2026-04-29] 实测后 Papers with Code 因死被 drop,scope 收敛到 22 adapter**(见 [`adapter-vs-skill.md §2`](./adapter-vs-skill.md) + [`adapter-empirical-test-report.md`](./adapter-empirical-test-report.md))
