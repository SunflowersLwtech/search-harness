---
source: arXiv API User's Manual - arXiv info
url: https://info.arxiv.org/help/api/user-manual.html
final_url: https://info.arxiv.org/help/api/user-manual.html
language: en
...
search for articles that contain the word
...
example, you can see the same search results for
...
Alternatively, you can search for articles that contain
...
sign in the constructed url's.

queryparameterstypedefaultsrequired
...
We have already seen the use of
...
is described in the search query construction
appendix. The
...
contains a comma-delimited list
of arXiv id's.
...
* If only
  ...
  is blank or not given), then the API will return results for each article that matches the search query.
* If only
  ...
  is blank or not given), then the API will return results for each article in
...
* If BOTH
  ...
  are given, then the API will return each article in
  ...
  . This allows the API to act as a results filter.
...
present
API returnsyesnoarticles that match

```
search_query
```

noyesarticles that are in
...
defines the index of the first returned result, using
0-based indexing.
...
is the number of results returned by
...
query parameters. For example to retrieve
matches 6001-8000:
http://export.arxiv.org/api/query?search\_query=all:electron&start=6000&max\_results=2000
...
OAI-PMH interface is more suitable. A request with

```
max_results
```

>30,000 will result in an HTTP 400 error code with
...
can be "relevance" (Apache Lucene's default RELEVANCE ordering), "lastUpdatedDate", "submittedDate"
...
An API response consists of an Atom
...
metadata about the API call performed, as well as child
...
element gives the title for the feed:
...
element serves as a unique id for this query, and is useful
...
element provides a URL that can be used to retrieve this
feed again.
...
provides a GET requestable url, even if the
...
element provides the last time the contents of the feed
were last updated:
...
element lists how many results are in the
...
results. The other two elements
...
element contains 0 or more child
...
representing an article in the
returned results set. As explained in the errors section, if
there are errors, a single

```
<entry>
```

element representing the error is
...
's representing arXiv articles. For a general discussion of
...
element contains the title of the article returned:
...
element contains a url that resolves to the abstract page for
that article:
...
tag contains the date in which the
...
version of
this article was submitted and processed. The
...
element contains the abstract for the article:
...
element for each author of the paper in order of
authorship. Each
...
element is used to describe either an arXiv, ACM, or
MSC classification. See the arXiv metadata
explanation for more details about these
classifications. The
...
,
which is the categorization scheme, and

```
term
```

which is the term used in
the categorization. Here is an example from the query
http://export.arxiv.org/api/query?id\_list=cs/9901002v1
...
For each entry, there are up to three
...
attributes. The table below summarizes what
these links refer to

reltitlerefers toalways presentalternate-abstract pageyesrelatedyesrelateddoiresolved doino
...
There are several pieces of arXiv metadata
...
http://export.arxiv.org/api/query?id\_list=cs/9901002v1, we have
...
is the primary arXiv classification for this
e-print.
...
element contains the typical author comments found
...
subelement of the standard Atom

```
<author>
```

element. For example, from the query
http://export.arxiv.org/api/query?id\_list=0710.5765v1, we have
...
for the error contains a helpful error message,
and the
...
element contains a url to a more detailed explanation
of the message.
...
Sample queryError Explanationhttp://export.arxiv.org/api/query?start=not\_an\_int
...
must be an integer
http://export.arxiv.org/api/query?start=-1
...
must be >= 0
http://export.arxiv.org/api/query?max\_results=not\_an\_int
...
must be an integer
http://export.arxiv.org/api/query?max\_results=-1
...
must be >= 0
http://export.arxiv.org/api/query?id\_list=1234.1234malformed id - see arxiv identifier explanationhttp://export.arxiv.org/api/query?id\_list=cond—mat/0709123malformed id - see arxiv identifier explanation
...
LanguageLibraryParsing ExamplePaging ExamplePerlXML::AtomparsingpagingPythonfeedparserparsingpagingRubyfeedtoolsparsingpagingPHPSimplePieparsingpaging

## 5. Appendices

### 5.1. Details of Query Construction

As outlined in the Structure of the API section, the
...
construction, and result set filtering through
...
's to retrieve our desired article lists. We outlined how
to use the
...
parameter to filter results sets in search\_query
and id\_list logic.
...
prefixexplanationtiTitleauAuthorabsAbstractcoCommentjrJournal ReferencecatSubject CategoryrnReport NumberidId (use
...
parameter should be used rather than

```
search_query=id:xxx
```

to properly handle article versions. In addition, note that

```
all:
```

searches in each of the fields simultaneously.

The API provides one date filter,

```
submittedDate
```

, that allow you to select data within a given date range of when the data was submitted to arXiv. The expected format is
...
is provided in 24 hour time to the minute, in GMT. We could construct the following query using
...
that also contain the word

```
checkerboard
```

in the title. We could construct the following query,
using the
...
in the title. Note that we included
...
signs in the
urls to the API. In a url, a

```
+
```

sign encodes a space, which is useful
...
programming libraries that deal with url's. Note that the
...
to see if you have escaped your url correctly.
...
Boolean operator is particularly useful, as it allows us to
filter search results based on certain fields. For example, if we wanted
all of the articles by the author
...
with titles that did not contain the words
...
This query returns three results. Notice that the
...
. For example, if we wanted all of the articles by the
author
...
This query returns one result, and notice that the feed
...
symbolencodingexplanation( )%28 %29Used to group Boolean expressions for Boolean operator precedence.double quotes%22 %22Used to group multiple words into phrases to search a particular field.space+Used to extend a
...
article, you may simply enter the arxiv id in the
...
is the version number you
are interested in.

For example, to retrieve the latest version of
...
, you
could use the query
http://export.arxiv.org/api/query?id\_list=cond-mat/0207270. To
retrieve the very first version of this article, you could use the query
http://export.arxiv.org/api/query?id\_list=cond-mat/0207270v1
...
elementexplanationfeed elements
...
The number of results returned.entry elements
...
of the article was submitted.

```
<updated>
```

The date that the retrieved version of the article was submitted. Same as

```
<published>
```

if the retrieved version is version 1.
...
One for each author. Has child element
...
The author's affiliation included as a subelement of
