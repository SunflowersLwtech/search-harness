---
source: REST API | GitLab Docs
url: https://docs.gitlab.com/api/rest/
final_url: https://docs.gitlab.com/api/rest/
language: en
...
* Query string:
  ...
  ```
  curl --request POST \ --url "https://gitlab.example.com/api/v4/projects?name=<example-name>&description=<example-description>"
  ```
* Request payload (JSON):

  ```
  curl --request POST \ --header "Content-Type: application/json" \ --data '{"name":"<example-name>", "description":"<example-description>"}' "https://gitlab.example.com/api/v4/projects"
  ```
...
The following example lists 50 namespaces per page:
...
The following example lists 50 projects per page, ordered
by
...
header contains the cursor value for retrieving the next page’s records,
while the
...
header contains the cursor value for retrieving the previous page’s,
when available.
...
N/A
Authenticated and unauthenticated users. Introduced in GitLab 17.1.
...
only.
Authenticated and unauthenticated users. Introduced in GitLab 18.3.
