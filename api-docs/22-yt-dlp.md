---
source: GitHub - yt-dlp/yt-dlp: A feature-rich command-line audio/video downloader
url: https://github.com/yt-dlp/yt-dlp
final_url: https://github.com/yt-dlp/yt-dlp
language: en
...
) contains ISC licensed code from
...
See THIRD\_PARTY\_LICENSES.txt for more details.
...
), the PyPI source distribution and the PyPI built distribution (wheel) only contain code licensed under the Unlicense.
...
* ```
  ...
  is the default channel, which offers releases published on a (mostly) monthly schedule. While it is named

  ```
  stable
  ```

  due to many of its changes having been tested by users of the
  ...
  release channels, the latest

  ```
  stable
  ```

  release is often "stale" and prone to external breakage (i.e. sites changing things on their end and breaking yt-dlp).
* The
  ...
  channel offers releases that publish shortly before midnight UTC on any day that sees changes to the codebase. This channel serves as a snapshot of the project's development, and it is the recommended channel for regular users of yt-dlp. The

  ```
  nightly
  ```

  releases are available from yt-dlp/yt-dlp-nightly-builds or as development releases of the
...
* The
  ...
  channel offers "canary" releases that publish after each push to the master branch. This channel will always provide the very latest fixes and features, but may be prone to bugs or regressions. The

  ```
  master
  ```

  releases are available from yt-dlp/yt-dlp-master-builds.
...
* curl\_cffi (recommended) - Python binding for curl-impersonate. Provides impersonation targets for Chrome, Edge and Safari. Licensed under MIT
...
* xattr, pyxattr or setfattr - For writing xattr metadata (
...
* mplayer or mpv - For downloading
...
file on a per-extractor basis. For that, you will need to create a
...
It may however also contain special sequences that will be replaced when downloading each video. The special sequences may be formatted according to Python string formatting operations, e.g.
...
* Replacement: A replacement value can be specified using a
  ...
  mini-language. If the field is not empty, this replacement value will be used instead of the actual field content. This is done after alternate fields are considered; thus the replacement is used if any of the alternative fields is not empty. E.g.
...
* ```
  ...
  (string): One of "not\_live", "is\_live", "is\_upcoming", "was\_live", "post\_live" (was live, but VOD is not yet processed)
* ```
  is_live
  ```
...
* ```
  ...
  (string): Whether the video is "private", "premium\_only", "subscriber\_only", "needs\_auth", "unlisted" or "public"
* ```
  media_type
  ```
...
* ```
  ...
  (string): Name of the video file. Note that the actual filename may differ
* ```
  formats_table
  ```
...
Note: Some of the sequences are not guaranteed to be present, since they depend on the metadata obtained by a particular extractor. Such sequences will be replaced with placeholder value provided with
...
* ```
  ...
  : Select the best quality format that contains either a video or an audio or both (i.e.;
...
* ```
  ...
  : Average bitrate of audio and video in kbps
* ```
  abr
  ```
...
is used to replace text in any metadata field using Python regular expression. Backreferences can be used in the replace string for advanced use.
...
is to give the name of a field or an output template to extract data from, and the format to interpret it as, separated by a colon

```
:
```

. Either a Python regular expression with named capture groups, a single field name, or a similar syntax to the output template (only
...
Note that these options preserve their relative order, allowing replacements to be made in parsed fields and vice versa. Also, any field thus created can be used in the output template and will also affect the media file's metadata added when using
...
* ```
  ...
  translated. See youtube/\_base.py for the list of supported content language codes
* ```
  skip
  ```
...
* ```
  ...
  (skip initial data/next ep request). While these options can help reduce the number of requests needed or avoid some rate-limiting, they could cause issues such as missing formats or metadata. See #860 and #12826 for more details
* ```
  webpage_skip
  ```
...
* ```
  ...
  : An HLS AES-128 key URI or key (as hex), and optionally the IV (as hex), in the form of
...
* Configuration directories: Plugin packages (containing a
  ...
  namespace folder) can be dropped into the following standard configuration locations:
  + User Plugins
    - ```
...
* pip and other locations in
  ...
  + Plugin packages can be installed and managed using
    ...
    . See yt-dlp-sample-plugins for an example.
...
* Merged with animelover1984/youtube-dl: You get most of the features and improvements from animelover1984/youtube-dl including
  ...
  , Embedding thumbnail in mp4/ogg/opus, playlist infojson etc. See #31 for details.
* YouTube improvements:
  ...
  + Supports Clips, Stories (
    ...
    ), Search (including filters)\*, YouTube Music Search, Channel-specific search, Search prefix (
...
* Improvements: Regex and other operators in
  ...
  , faster archive checking, more format selection options, merge multi-video/audio, multiple
...
* yt-dlp supports only Python 3.10+, and will remove support for more versions as they become EOL; while youtube-dl still supports Python 2.6+ and 3.2+
* The options
  ...
  ), no longer work. See removed options for details
* ```
  avconv
  ```
...
* ```
  ...
  . See #302 for details. You can use
...
* When
  ...
  to embed the subs and automatically delete the separate file. See #630 (comment) for more info.
...
* ~~yt-dlp tries to parse the external downloader outputs into the standard progress output if possible (Currently implemented: aria2c). You can use~~
...
* yt-dlp versions between 2021.09.01 and 2023.01.02 applies
  ...
  to nested playlists. This was an unintentional side-effect of 8f18ac and is fixed in d7b460. Use
...
* yt-dlp versions between 2021.11.10 and 2023.06.21 estimated
  ...
  values for fragmented/manifest formats. This was added for convenience in f2fe69, but was reverted in 0dff8e due to the potentially extreme inaccuracy of the estimated values. Use
...
* Passing
  ...
  ) no longer alters the default format selection. See #9843 for details.
...
* ```
  ...
  : Use all compat options (Do NOT use this!)
* ```
...
* ```
  ...
  : Allow files with any extension (including unsafe ones) to be downloaded (GHSA-79w7-vh3h-8g4j)⚠️Only use if a valid file download is rejected because its extension is detected as uncommon
...
Support for SponSkrub has been removed in favor of the
